...
func ProbeWithinTest(t *testing.T, system *actor.ActorSystem) *testProbe {
	...
	probe := &testProbe{t: t, system: system, message: make(chan interface{}, 1), timeout: time.Minute}
...
func (a *testProbe) ExpectUnorderedMessages(msg ...interface{}) {
	...
	go func() {
		var (
			...
			fail           = func() {
				done <- &failure{message: fmt.Sprintf("Expected %v but received %v", expectedMessages, receivedMessages)}
			}
			success = func() {
				done <- &success{}
			}
